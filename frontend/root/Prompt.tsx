...
declare global {
  ...
  function uploader(specs: {title: string, text: string, style?: any, maxmb?:number}): Promise<string>;
...
function Toast(props) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (document.getElementById("notifer"))
        document.getElementById("notifer").className = `${z.styles.notification} ${z.styles.show}`
    }, 200);
    const timeout = setTimeout(() => {
      if (document.getElementById("notifer"))
        document.getElementById("notifer").className = `${z.styles.notification} ${z.styles.hide}`
      setTimeout(() => {
        props.onfinish?.()
      }, to);
    }, to);
  ...
  return (
    <div id="notifer" className={`${z.styles.notification} ${z.styles.hide}`} style={{ backgroundColor: props.color }}>
      {props.message}
    </div>
  );
}
...
export default (props) => {
  ...
  window.uploader = (specs): Promise<string> => {
      setState({ show: "upload", title:specs.title, text:specs.text, maxmb:specs.maxmb, style:specs.style })
  ...
  if (!state.show) {
      ...
      <div style={{ maxHeight: 400, overflowX: "scroll" }}>
        {(state.items).map(st => {
          ...
          return <Icon2Titles title1={st.title1} style={{ backgroundColor: st.highlight ? "#61A75A" : "#B6C8B4", marginBottom: 1 }}
            icon={image}
            title2={st.imageprop?<Signature style={{ marginTop: 4 }}>{st.title2}</Signature>:<f-10>{st.title2}</f-10>}
    ...
    return <WindowFloat title={state.title || z.lang.sysmsg} maxWidth={300} onclose={() => {
      ...
      <f-cc class={!state.percent ? z.styles.none : z.styles.op1}>
        <Upload
          id={"propmpt-upload"}
          extensionfilter={[".jpg", ".png", '.jpeg', '.svg', '.webp']}
          maxsize={state.maxmb || 10 * 1024 * 1024} //10MB!
          singlefile
          // hidefileicons
      ...
      <br-x />
      {
        state.percent == 100 && state.url ? <f-cc class={z.styles.btnaccept}
    ...
    return <WindowFloat title={state.title || z.lang.sysmsg} onclose={() => { window["confirmresolve"](false); setState({ show: false }) }}
      ...
      <br-x />
      <f-cc class={z.styles.btnaccept} onClick={() => { window["confirmresolve"](true); setState({ show: false }) }}>{state.oktext ? state.oktext : z.lang.imsure}</f-cc>
      <br-xx />
      <f-cc class={z.styles.btncancel} onClick={() => { window["confirmresolve"](false); setState({ show: false }) }}>{state.canceltext ? state.canceltext : z.lang.cancel}</f-cc>
    </WindowFloat>
  }
    ...
    return <WindowFloat title={state.title || z.lang.sysmsg} style={{ direction: z.lang.dir }}
      ...
      {state.json ? <pre style={{ fontSize: 11, direction: "ltr" }}>
        ...
        <div style={{ zIndex: 1 }}>
          ...
          <br-x />
          <br-xx />
          <f-cc class={z.styles.btnaccept} onClick={() => { window["alertresolve"]?.(); setState({ show: false }) }}>{z.lang.confirm}</f-cc>
        </div>
